# UVBR-temperature-preference
Analyses of thermal preference and UVB exposure in tadpole amphibians

Author: Katlaina Gutiérrez Hernández

This repository contains the data and R scripts used to analyze the effects of ultraviolet-B radiation (UVBR) on temperature preference in tadpoles of three tropical anuran species: *Engystomops pustulosus*, *Boana platanera*, and *Rhinella horribilis*.

## 📁 Repository structure

- `scripts/stats/`  
  Statistical analyses evaluating the effects of UVBR and temperature.

- `scripts/figures/`  
  Scripts used to generate Figures 1–3 in different formats (color/black & white, horizontal/vertical).

- `data/`  
  Raw dataset containing species identity, experimental treatments, UVBR levels, and temperature measurements.

## ▶️ How to use

1. Load the dataset from the `data/` folder.
2. Run the statistical analysis script located in `scripts/stats/`.
3. Run the figure scripts in `scripts/figures/` to reproduce the figures.

## 📊 Requirements

- R
- Required packages:
  - `dplyr`
  - `ggplot2`
  - `readxl` (if using Excel files)

## 🔬 Experimental overview

The study evaluates how UVBR exposure influences thermal preference in tadpoles under controlled laboratory conditions, integrating multiple experimental treatments combining radiation levels and temperature.

